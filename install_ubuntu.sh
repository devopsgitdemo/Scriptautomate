...
yarn install
sleep 3
cd /etc/trudesk && npm run build && echo -e "${BLUE}Starting...${NC}" && pm2 start /etc/trudesk/app.js --name trudesk -l /etc/trudesk/logs/output.log --merge-logs && pm2 save && pm2 startup && echo -e "Installation & configuration successfully finished."
